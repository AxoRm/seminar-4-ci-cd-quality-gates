...
В репозитории есть маленькое приложение: `python main.py` печатает `Hello, world!`, а `shop.main:app` даёт маршруты `/`, `/health` и демонстрационный `/checkout`. Оплата имитируется. `requirements.txt` содержит библиотеку приложения, `requirements-dev.txt` — инструменты CI; код лежит в `src/shop`, тесты — в `tests`.

На GitHub реально работает только пример `.github/workflows/example-ci.yml`. `.gitlab-ci.yml` показывает проект большого pipeline; review/TEST/PERF/PROD-стенды ещё не развёрнуты, а маленький пример не доказывает выполнение лимита 10 минут для 1000 тестов.
