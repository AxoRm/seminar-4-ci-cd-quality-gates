...
flowchart TD
    ...
    TG -->|ошибка| STOP[Деплой TEST/PERF запрещён; исправить и запустить новый pipeline]
    T --> E[E2E: 150 / 50:00]
...
В нем одновременно начинаются: Линтер, статический анализ, сборка, unit;
Параллельно: integration и API после сборки.
Для E2E и нагрузки нужны **разные стенды**: нагрузка исказила бы результаты функциональных проверок на общем TEST.

Если gate перед TEST/PERF не пройден, оба деплоя блокируются до исправления и успешного нового pipeline на актуальном коммите.
