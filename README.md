...
**Материалы для защиты:** [текст выступления на 5–6 минут](Доклад_для_выступления.md) · [пример GitLab CI](.gitlab-ci.yml) · [проверка покрытия](ci/check_coverage.py) · [настройка coverage](.coveragerc).

Ниже — полное решение задания. Схема Mermaid и таблицы отображаются прямо на странице репозитория GitHub.

> `.gitlab-ci.yml` приведён как пример дополнительного задания для GitLab CI. GitHub используется для удобного просмотра и сдачи материалов; запуск примера требует backend-проекта и указанных в нём скриптов.
