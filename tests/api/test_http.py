...
def test_hello_http_response() -> None:
    assert client.get("/").json() == {"message": "Hello, world!"}


def test_health_http_response() -> None:
    assert client.get("/health").json() == {"status": "ok"}


def test_checkout_http_success() -> None:
...
def test_checkout_http_access_error() -> None:
    payload = {"user_id": 7, "owner_id": 8, "item_prices": [120, 80], "approved": True}
